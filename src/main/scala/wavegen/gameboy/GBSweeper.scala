...
class GBSweeper(implicit clockFreq: Int) extends Module {
	val io = IO(new Bundle {
		val tick      = Input(Bool()) // Sweep clock
		val trigger   = Input(Bool()) // Trigger event
		val negate    = Input(Bool()) // 0: addition, 1: subtraction
